...
this space he hasn't shared — scoped tightly to what agent-tui#116 actually asked for:
...
## Answering agent-tui#116's own first ask: what does "graphify" actually refer to

Two different things share the name, and agent-tui#116 explicitly asked not to let an
...
So "graphify," as a real, evaluable candidate for agent-tui#116's storage decision, is
...
category difference agent-tui#116's own three-way framing doesn't surface, and it
...
Ranked by directness to agent-tui#116's actual question, not by project popularity:
...
1. **Candidate-first import for any OKF bundle merged into the shared vault**
   ...
   change, not a storage decision — doesn't wait on agent-tui#116 at all.
...
3. **Bi-temporal fact validity** (`getzep/graphiti`, Apache-2.0, concept
   ...
   *when* it stopped being true, not only what replaced it, is a real gap
   worth closing independent of whatever agent-tui#116 decides.
...
5. **"graphify" is settled as a category, not a peer candidate** — it is an
   ...
   so agent-tui#116's three-way comparison should treat it as answering a different
...
- Does not decide agent-tui#116's storage format. That remains Jon's.
