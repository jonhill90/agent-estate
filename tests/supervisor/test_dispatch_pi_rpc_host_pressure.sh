#!/bin/bash
# dispatch-pi-rpc.sh must refuse a NEW dispatch when host_pressure.py
...
# test_dispatch_host_pressure.sh proves for dispatch.sh's tmux flow and
# test_dispatch_claude_print_host_pressure.sh proves for its claude-print
# sibling.
#
# WHY THIS EXISTS: agent-supervisor#643 measured, directly (grep for
# "host-pressure" across scripts/supervisor), that dispatch-pi-rpc.sh
# started a brand-new `pi --mode rpc` subprocess with NO host-pressure check
# of any kind -- host-pressure.sh's gate covered only dispatch.sh's tmux
# flow. This suite is the "a fix is not complete until a test that failed
# before it passes after it" evidence for that gap, applied to a new
# integration rather than a bugfix: a REFUSE case that must go RED if the
# gate call is ever removed from dispatch-pi-rpc.sh again.
set -uo pipefail
...
echo "dispatch-pi-rpc.sh -- the host-pressure gate (#643)"
...
cat > "$D/issues" <<'FIX'
643|| the pi-rpc host-pressure gate itself
645|| the pi-rpc host-pressure gate itself, mutation case
FIX
: > "$D/prs"
...
run() {
  ...
  PATH="$D/bin:$PATH" GH_ISSUES="$D/issues" GH_PRS="$D/prs" \
    ...
    "$@" \
    bash "${DISPATCH_SCRIPT:-$DISPATCH}" 643 "pi-rpc-hp-$n" "$D/brief.md" acme/agent-dotfiles "$REPO" 2>&1
}
...
# --- MUTATION: remove the gate call from dispatch-pi-rpc.sh -> REFUSE case
# must go RED. Proves the REFUSE assertion above is pinned to the script
# actually calling host_pressure.py, not to something else already
...
MUT_NOGATE="$HERE/../../scripts/supervisor/dispatch-pi-rpc-mutant-nogate.sh"
...
PY
if [ "$mut_rc" -ne 0 ]; then
  bad "setup: patched a copy of dispatch-pi-rpc.sh with the host-pressure call site disabled" \
    "could not patch $DISPATCH (exit $mut_rc)"
else
  ok "setup: patched a copy of dispatch-pi-rpc.sh with the host-pressure call site disabled"
  chmod +x "$MUT_NOGATE"
  echo '645|| a mutation case, issue never actually claimed by the real gate' > "$D/mut-issues"
  ...
  MUT_OUT=$(PATH="$D/bin:$PATH" GH_ISSUES="$D/issues" GH_PRS="$D/prs" \
    ...
    bash "$MUT_NOGATE" 645 pi-rpc-hp-mut "$D/brief.md" acme/agent-dotfiles "$REPO" 2>&1); MUT_RC=$?
  if [ "$MUT_RC" -eq 0 ]; then
...
echo
echo "dispatch-pi-rpc.sh host-pressure gate: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
