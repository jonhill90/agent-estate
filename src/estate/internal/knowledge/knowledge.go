...
//   - the operator's prompt/parameter corpus (~/corpus/corpus.sqlite3,
...
// agent-estate#1306: this used to end "-- regenerate with `estate
// knowledge` before trusting a count here over a live read of the
// source", which is displayed verbatim in the TUI's own knowledge-index
// dashboard (src/tui/internal/knowledgeindex/view.go) -- the same bare
// shared-index advice that #1306 found in the CLI's staleness prose,
...
type Config struct {
	...
	CorpusDBPath  string // ~/corpus/corpus.sqlite3 by default
