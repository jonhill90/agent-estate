#!/bin/bash
# agent-supervisor#104: one verified reap primitive, originally written for
# tests that start a real, long-lived poller-shaped process (a tmux-hosted
# stand-in or the real inbox-poll.sh under a throwaway LaunchAgent). Every
# such test hand-rolled its own kill line -- a bare `pkill -KILL -f
# "$STAND_IN"`, fire-and-forget, that never confirms the process is actually
# gone and never reports if it is not. #104 measured two of these processes
# outliving their test by hours: both survived SIGTERM and needed SIGKILL,
# and both were reaped by hand because nothing automated ever tried the
# escalation.
#
# agent-supervisor#382: moved here from tests/supervisor/lib/reap-verified.sh
# (which now just sources this file) so poller-leak-cleanup.sh -- a
# production tool, not a test -- can reuse the exact same verified-kill
# behaviour instead of a second hand-rolled kill loop. Nothing about the
# primitive changed; only its address did.
#
...
#                       the caller (read from a PID_HISTORY/PID_FILE or a
#                       process listing the caller trusts).
#   sandbox-substring   a path fragment that must appear in the process's own
#                       command line (its script path) before this function
...
# rightly refused as out-of-scope and independently still not this caller's
# to kill; returns the worst (highest) individual exit status otherwise so a
