...
# for a set of decoy windows before and after agent-tui runs in its OWN
# window of the same session.
#
# Usage: verify-lanes-unaffected.sh <path-to-agent-supervisor-checkout> [agent-tui binary]
#
...
SUPERVISOR_REPO="${1:?usage: verify-lanes-unaffected.sh <agent-supervisor-repo> [agent-tui-binary]}"
BINARY="${2:-agent-tui}"
...
if ! command -v "$BINARY" >/dev/null 2>&1 && [ ! -x "$BINARY" ]; then
  echo "verify-lanes-unaffected: agent-tui binary not found: $BINARY (build it first: go build -o agent-tui ./cmd/agent-tui)" >&2
  exit 1
fi
...
# Only the windows that existed before agent-tui ran are part of the claim --
# agent-tui's own window is expected to appear as a new row (it is a real
...
filter_to_before() {
  ...
  # agent-tui exists -- the wait between BEFORE and AFTER moves it on its
...
# agent-tui in ITS OWN window of the same isolated session -- never a pane
...
tmux new-window -d -t "$SESSION" -n agent-tui-under-test \
...
if [ "$BEFORE" = "$AFTER" ]; then
  echo "PASS: lanes.sh --json is byte-identical for pre-existing windows with agent-tui running"
  exit 0
fi

echo "FAIL: lanes.sh --json changed for a pre-existing window while agent-tui ran" >&2
